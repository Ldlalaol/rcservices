...
def kb_main():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="🗑 Вынести мусор"), KeyboardButton(text="🆘 Помощь")],
    ], resize_keyboard=True)

def kb_nav():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="◀️ Назад"), KeyboardButton(text="❌ Отменить заявку")],
    ], resize_keyboard=True)
...
def kb_trash():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="🏠 Бытовой")],
        [KeyboardButton(text="🧱 Строительный"), KeyboardButton(text="📦 Крупногабаритный")],
        [KeyboardButton(text="◀️ Назад"), KeyboardButton(text="❌ Отменить заявку")],
    ], resize_keyboard=True)

def kb_time():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="⚡ Сейчас"), KeyboardButton(text="🕐 В течение часа")],
...
def kb_comment():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="💬 Добавить комментарий"), KeyboardButton(text="➡️ Без комментария")],
        [KeyboardButton(text="◀️ Назад"), KeyboardButton(text="❌ Отменить заявку")],
    ], resize_keyboard=True)
...
def kb_confirm():
    return ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton(text="✏️ Изменить"), KeyboardButton(text="❌ Отменить заявку")],
    ], resize_keyboard=True)

def kb_price_confirm():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="✅ Принять цену")],
        [KeyboardButton(text="❌ Отменить заявку")],
    ], resize_keyboard=True)

def kb_review():
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="⏭ Пропустить отзыв")],
    ], resize_keyboard=True)
...
async def go_to_comment(message: Message, state: FSMContext):
    ...
    await message.answer("💬 Хотите добавить комментарий к заявке?", reply_markup=kb_comment())

async def go_to_time(message: Message, state: FSMContext):
    await state.set_state(Order.choosing_time)
    await message.answer("⏰ Когда вы хотите принять заказ?", reply_markup=kb_time())
...
async def ask_photo(message: Message, state: FSMContext):
    ...
    if trash == "🏠 Бытовой":
        ...
        note = "💰 Стоимость рассчитывается сотрудником по фото." if bags > 10 else f"💰 Стоимость вывоза: <b>{price}</b>"
        await message.answer(f"{note}\n\n📸 Пришлите фото мусора:", reply_markup=kb_nav())
    else:
...
async def send_order_to_worker(bot: Bot, data: dict, order_id: str, needs_price: bool):
    ...
    text = (
        f"📬 <b>Новая заявка!</b>\n\n{order_summary(data)}\n\n"
        + ("💰 <b>Укажите цену для клиента:</b>" if needs_price
           else "ℹ️ Цена фиксированная. Можете приступать!")
    )
...
async def worker_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "По заявкам с оценкой — нажмите <b>«💰 Указать цену»</b>.\n"
...
async def worker_set_price_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    if not order or order["status"] != "pending":
        await callback.answer("❌ Заявка не найдена или уже закрыта.", show_alert=True)
        return
    await state.set_state(Worker.entering_price)
    await state.update_data(pricing_order=order_id)
    await callback.message.answer(f"💰 Введите цену (₸) для заявки <code>{order_id}</code>:")
...
@client_router.message(IsClient(), F.text == "🆘 Помощь")
async def help_handler(message: Message):
    await message.answer("📞 <b>Служба поддержки ЖК</b>\n\nТелефон: +7 (XXX) XXX-XX-XX\nВремя работы: 09:00 — 18:00")
...
async def cancel_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    if current_state == Order.price_confirm.state and order_id:
        ...
        await bot.send_message(WORKER_ID, f"❌ Клиент отказался от заявки <code>{order_id}</code> после указанной цены.")
    elif order_id:
        await update_order_status(order_id, "canceled")

    await state.clear()
    await state.set_state(Order.choosing_service)
    await message.answer("❌ Заявка отменена. Возвращаемся в главное меню.", reply_markup=kb_main())
...
async def skip_review(message: Message, state: FSMContext):
    ...
    await state.clear()
    await state.set_state(Order.choosing_service)
    await message.answer("Спасибо! Возвращаемся в главное меню.", reply_markup=kb_main())
...
async def process_review(message: Message, state: FSMContext, bot: Bot):
    ...
    if not review:
        await message.answer("Напишите отзыв текстом или нажмите «⏭ Пропустить отзыв».", reply_markup=kb_review())
        return
    ...
    await state.clear()
    await state.set_state(Order.choosing_service)
    await message.answer("Спасибо за отзыв! Возвращаемся в главное меню.", reply_markup=kb_main())
...
# ── Услуга ────────────────────────────────────────────────
@client_router.message(IsClient(), Order.choosing_service, F.text == "🗑 Вынести мусор")
async def garbage_service(message: Message, state: FSMContext):
    ...
    await message.answer("🏢 Укажите номер вашего блока:", reply_markup=kb_blocks())

# ── Блок ──────────────────────────────────────────────────
@client_router.message(IsClient(), Order.choosing_block, F.text == "◀️ Назад")
async def block_back(message: Message, state: FSMContext):
    await state.set_state(Order.choosing_service)
    await message.answer("Выберите услугу:", reply_markup=kb_main())
...
# ── Этаж ──────────────────────────────────────────────────
@client_router.message(IsClient(), Order.entering_floor, F.text == "◀️ Назад")
async def floor_back(message: Message, state: FSMContext):
    data = await state.get_data()
    if data.get("editing"):
        await state.update_data(editing=False); await show_confirm(message, state); return
    await state.set_state(Order.choosing_block)
    await message.answer("🏢 Укажите номер блока:", reply_markup=kb_blocks())
...
async def process_floor(message: Message, state: FSMContext):
    ...
    if not text.isdigit() or not (1 <= int(text) <= 30):
        await message.answer("🚫 Введите число от 1 до 30:"); return
...
# ── Тип мусора ────────────────────────────────────────────
@client_router.message(IsClient(), Order.choosing_trash, F.text == "◀️ Назад")
...
@client_router.message(IsClient(), Order.choosing_trash, F.text.in_(TRASH_TYPES))
...
async def trash_invalid(message: Message):
    await message.answer("⚠️ Выберите тип мусора с помощью кнопок выше.")

# ── Кол-во пакетов ────────────────────────────────────────
@client_router.message(IsClient(), Order.entering_bags, F.text == "◀️ Назад")
...
async def process_bags(message: Message, state: FSMContext):
    text = message.text.strip()
    if not text.isdigit() or int(text) <= 0:
        await message.answer("🚫 Введите корректное количество пакетов:"); return
    bags = int(text)
...
# ── Фото ──────────────────────────────────────────────────
@client_router.message(IsClient(), Order.sending_photo, F.text == "◀️ Назад")
async def photo_back(message: Message, state: FSMContext):
    ...
    if data.get("trash_type") == "🏠 Бытовой":
        await state.set_state(Order.entering_bags)
        await message.answer("📦 Введите количество пакетов:", reply_markup=kb_nav())
    else:
        await state.set_state(Order.choosing_trash)
        await message.answer("🗑 Выберите тип мусора:", reply_markup=kb_trash())
...
async def photo_invalid(message: Message):
    await message.answer("📸 Пожалуйста, отправьте <b>фотографию</b> мусора.")

# ── Время ─────────────────────────────────────────────────
@client_router.message(IsClient(), Order.choosing_time, F.text == "◀️ Назад")
...
@client_router.message(IsClient(), Order.choosing_time, F.text.in_({"⚡ Сейчас", "🕐 В течение часа"}))
async def process_time_preset(message: Message, state: FSMContext):
    label = "Сейчас" if message.text == "⚡ Сейчас" else "В течение часа"
...
@client_router.message(IsClient(), Order.choosing_time, F.text == "🕒 Указать время")
async def process_time_custom(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Доступное время: 09:00 – 18:00.", reply_markup=kb_nav(),
    )

@client_router.message(IsClient(), Order.choosing_time)
async def time_invalid(message: Message):
    await message.answer("⚠️ Выберите вариант с помощью кнопок выше.")

@client_router.message(IsClient(), Order.entering_time, F.text == "◀️ Назад")
...
async def process_entering_time(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("🚫 Неверный формат. Введите как <b>ЧЧ:ММ</b>, например 14:30:"); return
    if not (9 <= h < 18):
        await message.answer("🚫 Время должно быть в диапазоне <b>09:00 – 18:00</b>:"); return
    today = datetime.now().strftime("%d.%m.%Y")
...
# ── Комментарий ───────────────────────────────────────────
@client_router.message(IsClient(), Order.asking_comment, F.text == "◀️ Назад")
...
@client_router.message(IsClient(), Order.asking_comment, F.text == "➡️ Без комментария")
...
@client_router.message(IsClient(), Order.asking_comment, F.text == "💬 Добавить комментарий")
async def ask_comment_text(message: Message, state: FSMContext):
    await state.set_state(Order.entering_comment)
    await message.answer("✏️ Напишите ваш комментарий:", reply_markup=kb_nav())

@client_router.message(IsClient(), Order.asking_comment)
async def comment_ask_invalid(message: Message):
    await message.answer("⚠️ Выберите вариант с помощью кнопок выше.")

@client_router.message(IsClient(), Order.entering_comment, F.text == "◀️ Назад")
...
async def edit_order(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    await message.answer("✏️ Что именно хотите изменить?", reply_markup=kb_edit(data))

# ── Клиент принимает цену ─────────────────────────────────
@client_router.message(IsClient(), Order.price_confirm, F.text == "✅ Принять цену")
async def client_accept_price(message: Message, state: FSMContext, bot: Bot):
    ...
    if not order or order["status"] != "price_sent":
        await state.clear()
        await state.set_state(Order.choosing_service)
        await message.answer("❌ Заявка не найдена или уже закрыта. Возвращаемся в главное меню.", reply_markup=kb_main())
        return
...
# ── Редактирование ────────────────────────────────────────
@client_router.message(IsClient(), Order.editing, F.text == "◀️ Назад")
async def edit_back(message: Message, state: FSMContext):
    await show_confirm(message, state)

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить блок")
async def edit_block(message: Message, state: FSMContext):
    ...
    await message.answer("🏢 Выберите новый блок:", reply_markup=kb_blocks())

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить этаж")
async def edit_floor(message: Message, state: FSMContext):
    ...
    await message.answer("🪜 Введите новый этаж:", reply_markup=kb_nav())

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить квартиру")
async def edit_apt(message: Message, state: FSMContext):
    ...
    await message.answer("🚪 Введите новый номер квартиры:", reply_markup=kb_nav())

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить тип мусора")
async def edit_trash(message: Message, state: FSMContext):
    ...
    await message.answer("🗑 Выберите новый тип мусора:", reply_markup=kb_trash())

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить кол-во пакетов")
async def edit_bags(message: Message, state: FSMContext):
    ...
    await message.answer("📦 Введите новое количество пакетов:", reply_markup=kb_nav())

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить фото")
async def edit_photo(message: Message, state: FSMContext):
    await state.update_data(editing=True, photo_id=None)
    await ask_photo(message, state)

@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить время")
...
@client_router.message(IsClient(), Order.editing, F.text == "🔄 Изменить комментарий")
async def edit_comment(message: Message, state: FSMContext):
    ...
    await message.answer("💬 Хотите добавить комментарий?", reply_markup=kb_comment())
